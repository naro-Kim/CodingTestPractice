...
# 1. 문제 조건
#   - 배열에 자연수 x를 넣는다.
#   - 입력이 '0'이면 배열에서 가장 큰 값을 출력한다. 배열이 비었으면 '0' 출력
...
#   - 최소힙의 노드 데이터에 '-1'을 곱하면 최대힙 구조가 된다.
#
# 4. 최대힙 구현
#   - 1) 파이썬에서 heapq에 (-입력값, 입력값)의 튜플 형태로 데이터를 넣는다.  
#   - 2) 노드가 '-'부호가 붙은 입력값을 기준으로 정렬되고, 튜플의 [1]번째 item인 원래 입력값이 노드의 데이터가 된다. 
